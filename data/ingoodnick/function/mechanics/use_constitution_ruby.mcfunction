...
# If the player has more than 10 hearts, they can't use the ruby anymore so we back out early.
execute if score @s ingoodnick_player_hp matches 20.. run function ingoodnick:mechanics/exit_early_on_use {item:"constitution_ruby"}
execute if score @s ingoodnick_player_hp matches 20.. run return 0

# We boost player HP by 2 for each ruby. Note I would typically use iterative macros here, but that would require
...
execute if score @s ingoodnick_player_hp matches 17..19 run function ingoodnick:mechanics/set_hp {hp:20,damage:17}

# Warning for the edge case, could potentially add an advancement here?
execute if score @s ingoodnick_player_hp matches 20 run title @s actionbar {translate:"item.ingoodnick.constitution_gem.no_more_uses",fallback:"Your are at your mortal end, the jewel can empower you no more..."}
